...
const Layout = () => {
    ...
    const headerDisplay =
      location?.pathname === "/xyz" || location?.pathname === "/abc"
        ? "hidden"
        : "block";
    ...
    return (
        <>
            {location?.pathname === "/aviator" ? <AvitatorLayout component={<AviatorHome />} /> : <div className="flex h-[100dvh]">
                <div
                    className={`${bgColor} shrink-0 bg-mainBg flex flex-col h-screen overflow-hidden  w-full text-white `}
                >
                    <div className={`h-[3.22rem]  w-full z-50 fixed top-0 `}>
                        <Header />
                    </div>
                    <div
                    ...
                    <div className="h-[3.5rem] bg-[#041027] flex items-center lg:hidden w-full z-50 fixed bottom-0">
                    ...
                    <div className="fixed inset-0 bg-black bg-opacity-80 z-50 flex items-center justify-center">
                        <div className="w-full h-full bg-mainBg text-white p-2">
                            <div className="flex items-center justify-between ">
                                <h1 className="text-xsm ">{myDetails?.data?.wallet}{currency?.uae}</h1>
                                <button onClick={() => setActiveModal(null)} className=" text-3xl text-textGray2 hover:text-white ">&times;</button>
                            </div>
                            <BetSlipHome />
                        </div>
                    </div>
                    ...
                    <div className="fixed inset-0 bg-opacity-80 z-50 flex items-end justify-center">
                        ...
                        <div className="w-full h-full bg-mainBg text-white">
                            <div className="flex items-center justify-between border-bg2 border-b-[0.5px] p-2">
                                ...
                                <button onClick={() => setActiveModal(null)} className=" text-3xl text-white hover:text-white ">&times;</button>
                            </div>
                            <MenuList />
                        </div>
                    </div>
