...
function NotificationModal({ setIsWalletModal }) {
  ...
  const [betAmounts, setBetAmounts] = useState(["100000", "200000", "100000"]);
  const [isEditing, setIsEditing] = useState(false);

  const [editableBetAmounts, setEditableBetAmounts] = useState([...betAmounts]);
  ...
  const handleInputChange = (index, value) => {
    const newAmounts = [...editableBetAmounts];
    newAmounts[index] = value;
    setEditableBetAmounts(newAmounts);
  };
        ...
        <div className="fixed mt-[10rem] inset-0 z-500 flex justify-center items-center">
          ...
          >
            ...
            <div className="mt-3">
              ...
              <div className="bg-white/5 mt-[2px] rounded-[4px] w-full h-[3rem]">
                <div className="flex items-center w-full gap-1 h-full ps-2 pe-2">
                  {betAmounts.map((amount, index) => (
                    isEditing ? (
                      <input
                        ...
                        className="h-[36px] w-full rounded-[4px] bg-white/10 text-white px-2 text-[13px]"
                      />
                    ) : (
                      <button
                        key={index}
                        className="h-[36px] bg-white/10 w-full rounded-[4px] text-white"
                      >
                        {amount}
                      </button>
                    )
                  ...
                  <div
                    className="h-[36px] bg-white/10 w-full rounded-[4px] flex justify-center items-center cursor-pointer"
                    onClick={() => setIsEditing(!isEditing)}
                  >
                    <MdModeEdit className="text-[24px] text-white" />
                  </div>
                </div>
              </div>
